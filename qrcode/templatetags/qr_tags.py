...
def qr_from_text(context, text, size='M', class=''):
    ...
    return {'text': text, 'size': actual_size, 'class': class}


@register.inclusion_tag('qrcode/qr_tag.html', takes_context=True)
def qr_from_mail(context, text, size='M', class=''):
    return qr_from_text(context, text='mailto:%s' % text, size=size, 'class': class)


@register.inclusion_tag('qrcode/qr_tag.html', takes_context=True)
def qr_from_contact(context, contact, size='M', class=''):
    final_string = 'MECARD:'
    if contact['name']:
    ...
    if contact['company']:
        final_string += 'ORG:%s;' % contact['company'].replace(
            ' ', '+')
    return qr_from_text(context, text=final_string, size=size, 'class': class)


@register.inclusion_tag('qrcode/qr_tag.html', takes_context=True)
def qr_from_object(context, obj, size='M', class=''):
    domain = Site.objects.get_current().domain
    path = obj.get_absolute_url()
    text = 'http://%s%s' % (domain, path)
    return qr_from_text(context, text, size=size, 'class': class)
